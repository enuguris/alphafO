...
const N: Record<NodeId, NodeMeta> = {
  ...
  risk:      { x:220, y:232, w:152, h:76,   label:'Risk Gate',         sub:'4 circuit breakers · trailing stop · 3% heat cap (₹15k)',    color:'#ff5252' },
  ...
  celery:    { x:408, y:340, w:224, h:74,   label:'Celery Workers',    sub:'15 scheduled tasks · Beat scheduler · last-run tracked',      color:'#26c6da' },
...
const DETAILS: Record<NodeId, Detail> = {
  ...
  risk: {
    ...
    overview:'Every trade order passes through the risk gate before execution. Four independent circuit breakers live in Redis — no DB round-trips on the hot path. State re-syncs from DB on every startup. Risk parameters stored as percentages (3.0 = 3%) in .env.',
    bullets:[
      'DAILY_PNL — realized P&L < −2% of capital → TRADING_HALTED set to "1" immediately',
      'Portfolio heat — deployed capital capped at 3% of ₹5L = ₹15,000; config stored as percent not fraction',
      ...
      'Expiry-safe — max_pain and expiry_week are whitelisted; they THRIVE near expiry',
    ],
  ...
  celery: {
    ...
    tagline:'15 scheduled tasks · Celery Beat · Redis broker · Runs independently of FastAPI',
    overview:'Distributed task queue running independently of FastAPI. Heavy jobs (scanning, backtesting, data sync) never block API responses. Celery Beat provides cron-like scheduling. Each task stamps its completion time to Redis so /system/schedule shows last-run timestamps.',
    bullets:[
      'scan-priority-15m (*/15 min) — runs all 8 patterns; deduplicates (ACTIVE key, no time limit); auto-executes ≥82% conf as paper trades',
      'scan-all-1h (*/1h) — full universe scan across 1h, 4h, daily timeframes',
      'mtm-update (*/2 min) — marks all open trades to market; applies trailing stop at +30% gain',
      'expiry-settlement (*/10 min) — closes trades whose options have passed expiry date',
      ...
      'nightly-pattern-discovery (02:00 daily) — Mann-Whitney U miner on bhav history for new edges',
      'reset-daily-pnl (09:15 daily) — clears Redis P&L + deployed capital; resyncs heat from DB',
      'reset-weekly-pnl (Monday 09:15) — clears weekly_pnl field in portfolio table',
      'confirm-order-fills (*/1 min) — polls Kite for pending live trade fills; cancels stale orders',
