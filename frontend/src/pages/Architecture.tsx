...
const N: Record<NodeId, NodeMeta> = {
  ...
  backend:   { x:28,  y:106, w:278, h:102,  label:'FastAPI Backend',   sub:'Python 3.12 · async · 10 API routes · WS hub · Redis spot cache', color:'#26c6a0' },
  ...
  risk:      { x:220, y:232, w:152, h:76,   label:'Risk Gate',         sub:'4 circuit breakers · trailing stop · dynamic heat cap',    color:'#ff5252' },
  ...
  redis:     { x:220, y:340, w:152, h:74,   label:'Redis',             sub:'4 circuit-breaker keys · Celery broker · spot:NIFTY cache',  color:'#ff7043' },
  celery:    { x:408, y:340, w:224, h:74,   label:'Celery Workers',    sub:'16 scheduled tasks · Beat scheduler · health-scan every 5m', color:'#26c6da' },
...
const DETAILS: Record<NodeId, Detail> = {
  ...
  patterns: {
    ...
    bullets:[
      ...
      'Signal dedup — (underlying, pattern, direction, option_type) with no time limit (ACTIVE key)',
      'Direction flip — opposite-direction signal expires all ACTIVE signals for same pattern bucket',
      'Age gate — signals > 2h old skipped at auto-execution to avoid stale strikes',
      'BUY_PATTERNS: gap_fill, oi_buildup, vwap_oi, pcr_divergence, mean_reversion, max_pain — buy CE for long, buy PE for short',
      'SELL_PATTERNS: iv_crush, expiry_week — direction-neutral premium sellers, sell OTM to collect theta',
      'option_strategy field (buy/sell) determines trade action — NOT signal.direction alone',
      'Regime OHLCV uses real FUTIDX prices from 253 cached bhav files (daily TF); synthetic for intraday',
      'estimated_premium uses real Black-Scholes with actual expiry DTE (not hardcoded 7 days)',
    ],
    tech:['Python', 'NumPy', 'SciPy', 'Black-Scholes', 'yfinance (OHLCV fallback)', 'NSE bhavcopy'],
    connects:[
      'FastAPI — scanner orchestrated by scan-priority-15m Celery task every 15 min',
  ...
  risk: {
    ...
    tagline:'4 Redis circuit breakers · Dynamic risk params · Portfolio heat cap · Kill switch',
    overview:'Every trade order passes through the risk gate before execution. Four independent circuit breakers live in Redis — no DB round-trips on the hot path. All risk parameters are dynamically configurable via PUT /api/v1/options/risk/params and the Settings UI — .env values are defaults only. State re-syncs from DB on every startup and every 5 minutes via health-scan.',
    bullets:[
      'DAILY_PNL — realized P&L < −max_daily_loss_pct% of capital → TRADING_HALTED set to "1"',
      'Portfolio heat — deployed capital capped at max_portfolio_heat% of paper_capital (Redis: alphafO:risk_params)',
      'max_concurrent_trades — additional gate in _auto_paper_trade; default 10',
      'Kill switch — KILL_SWITCH_KEY permanent halt until manually cleared; health-scan auto-clears after 30m if loss recovered',
      'Dynamic params — GET/PUT/DELETE /api/v1/options/risk/params; changes survive restarts via Redis',
      ...
      'health-scan — every 5 min auto-corrects Redis drift > ₹5,000; expires stale signals; auto-clears old halts',
      'Event risk block — non-expiry patterns skip auto-execution on RBI/FOMC/monthly-expiry days',
    ],
    tech:['Redis 7', 'redis-py async', 'Python'],
    connects:[
      'Redis — alphafO:risk_params (dynamic overrides), DAILY_PNL, DAILY_DEPLOYED, TRADING_HALTED, KILL_SWITCH',
      'FastAPI — gate checked before every auto-trade; PUT /risk/params updates Redis live',
    ],
  },
  ...
  options: {
    ...
    bullets:[
      ...
      'IV rank — (current - 52wk_min) / (52wk_max - 52wk_min); history spans 10–31% for NIFTY',
      'IV rank reads ATM chain IV (not random); history seeded with realistic 252-day spread',
      'Max pain — strike minimising total OI-weighted option writer loss; computed from NSE bhavcopy OI',
      'Expiry calendar — weekly Tuesday for NIFTY/BANKNIFTY (NSE changed from Thursday in 2025)',
      'Event calendar — results dates, RBI MPC, FOMC, monthly NSE expiry; dte=1 triggers risk block',
      'Strike selector — ATM ± N strikes based on signal direction, DTE, and lot size constraints',
      'Hedge leg — SELL trades get an OTM BUY hedge; only created if hedge_prem < 85% of main premium',
      'Regime — prefers real bhav FUTIDX prices (253 days) over synthetic random walk; uses real India VIX',
      'MTM repricing — ATM chain IV used (not hardcoded 18%); options repriced with actual expiry DTE',
    ],
  ...
  redis: {
    ...
    tagline:'4 circuit-breaker keys · Celery broker · spot: price cache for Celery MTM',
    overview:'In-memory store for real-time risk state, Celery task brokering, and cross-process spot price cache. State resets at 09:15 IST daily. All risk checks are in-memory — zero DB latency on the trade hot path. KiteTicker writes spot:{SYM} on every tick so Celery MTM workers get real prices without a running WebSocket.',
    bullets:[
      ...
      'spot:{SYM} — written by KiteTicker on every tick; TTL=1h; Celery reads this for BS option pricing',
      'task_last_run:{label} — each Celery task stamps ISO timestamp so /system/schedule shows last-run',
      'scan-all-1h/eod/premarket each stamp their own label (not shared) so timestamps are distinct',
      'Celery broker — Redis queues all 15 task messages; stores task results (result_backend)',
    ...
    connects:[
      ...
      'Celery — message broker for all 15 tasks; result storage',
  ...
  celery: {
    ...
    tagline:'16 scheduled tasks · Celery Beat · Redis broker · Runs independently of FastAPI',
    overview:'Distributed task queue running independently of FastAPI. Heavy jobs (scanning, backtesting, data sync) never block API responses. Celery Beat provides cron-like scheduling. Each task stamps its completion time to Redis so /system/schedule shows last-run timestamps. Composite multi-leg trades span two different expiry dates; strategy type (Diagonal/Calendar/Condor) auto-selected by IV rank.',
    bullets:[
      'scan-priority-15m (*/15 min, 9–15 IST) — runs all 8 patterns; deduplicates; auto-executes ≥82% conf as composite paper trades',
      'health-scan (*/5 min, all day) — checks Redis drift >₹5k, expires stale signals >2h, auto-clears halts older than 30m, warns if signal queue empty',
      'mtm-update (*/1 min, 9–15 IST) — marks all open trades to market via spot:{SYM} Redis keys; applies trailing stop at +30% gain',
      'scan-all-1h (05 min past each hour, 9–15 IST) — full universe scan across 1h, 4h, daily timeframes',
      'cleanup-stale-signals (*/15 min) — expires ACTIVE signals past valid_until; purges EXPIRED records',
      'eod-close-intraday (15:20 IST Mon–Fri) — force-closes intraday trades before auto-square-off',
      'expiry-settlement (15:31 IST Mon–Fri) — closes trades whose options have passed expiry date',
      'sync-market-data (16:15 IST Mon–Fri) — downloads NSE bhavcopy; bootstraps PCR + max-pain cache',
      'nightly-pattern-backtest (16:00 daily) — walk-forward backtests all discovered patterns',
      'nightly-pattern-discovery (16:30 daily) — statistical pattern miner on bhav history for new edges',
      'reset-daily-pnl (09:15 daily) — clears Redis P&L + deployed capital; resyncs heat from DB',
      'generate-briefing (08:45 IST Mon–Fri) — calls Anthropic Claude with PCR, FII, India VIX, IV rank',
      'verify-lot-sizes (08:30 IST Mon–Fri) — cross-checks NIFTY/BANKNIFTY lot sizes vs Kite NFO master',
    ],
    tech:['Celery 5', 'Celery Beat', 'Redis broker', 'Python 3.12', 'Anthropic SDK'],
    connects:[
      'Redis — all task messages queued here; health-scan corrects DAILY_DEPLOYED drift',
      'PostgreSQL — patterns, backtest runs, briefings, composite trades stored here',
